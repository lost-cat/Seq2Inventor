...
def create_CAD(cad_seq: CADSequence):
    """create a 3D CAD encoder from CADSequence. Only support extrude with boolean operation."""
...
def create_profile_face(profile: Profile, sketch_plane: CoordSystem):
    """create a face from a sketch_inventor profile and the sketch_inventor plane"""
...
def create_loop_3d(loop: Loop, sketch_plane: CoordSystem):
    """create a 3D sketch_inventor loop"""
...
def point_local2global(point, sketch_plane: CoordSystem, to_gp_Pnt=True):
    """convert point in sketch_inventor plane local coordinates to global coordinates"""

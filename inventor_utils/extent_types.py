...
class ExtentFactory:
    ...
    @staticmethod
    def from_dict(d: dict) -> ExtentWrapper:
        extent_type = d.get("type", "")
        if extent_type == "AngleExtent":
            return AngleExtentWrapper.from_dict(d)
        elif extent_type == "DistanceExtent":
            return DistanceExtentWrapper.from_dict(d)
        elif extent_type == "ToExtent":
            return ToExtentWrapper.from_dict(d)
        elif extent_type == "FullSweepExtent":
            return FullSweepExtentWrapper.from_dict(d)
        elif extent_type == "ThroughAllExtent":
            return ThroughAllExtentWrapper.from_dict(d)
        elif extent_type == "FromToExtent":
            return FromToExtentWrapper.from_dict(d)
        elif extent_type == "ToNextExtent":
            return ToNextExtentWrapper.from_dict(d)
        else:
            raise ValueError(f"Unsupported extent type in dict: {extent_type}")
...
class AngleExtentWrapper(ExtentWrapper):
    ...
    def extract_kvs(self) -> dict:
        ...
        return {
            "nums": {
                KEY["ANGLE"]: self.angle.value,
            },
            "enums": {
                KEY["DIR"]: DIR_ID.get(self.direction, 1),
            },
            "entities": {},
        }
...
class DistanceExtentWrapper(ExtentWrapper):
    ...
    def from_dict(cls, d: dict):
        instance = cls()
        instance.distance = Parameter.from_dict(d.get("distance", {}))
        instance.direction = d.get("direction", "")
        return instance
...
class ToExtentWrapper(ExtentWrapper):
    ...
    def from_inventor_extent(cls, extent):  # type: ignore
        ...
        is_faces_collection = hasattr(to_entity, "Count") and hasattr(to_entity, "Item")
    ...
    def from_dict(cls, d: dict):
        instance = cls()
        instance.to_entity = d.get("toEntity", {})
        instance.direction = d.get("direction", "")
        instance.extend_to_face = d.get("extendToFace", False)
        return instance
    ...
    def extract_kvs(self) -> dict:
        return {
            ...
            "enums": {
                KEY["DIR"]: DIR_ID.get(self.direction, 1),
                KEY["IS_EXTEND_TO_FACE"]: 1 if self.extend_to_face else 0,
            },
            "entities": {
                KEY["TOFACE_ID"]: self.to_entity,
            },
        }
...
class FromToExtentWrapper(ExtentWrapper):
    ...
    def from_inventor(cls, extent):  # type: ignore
        ...
        if is_type_of(instance.inventor_extent.FromFace, "Face") is True:
            instance.from_face = collect_face_metadata(
                instance.inventor_extent.FromFace
            )
            instance.is_from_face_work_plane = False
        else:
            instance.from_face = PlaneEntityWrapper.from_work_plane(
                instance.inventor_extent.FromFace, None
            ).to_dict()
        ...
        if is_type_of(instance.inventor_extent.ToFace, "Face") is True:
            ...
            instance.to_face = PlaneEntityWrapper.from_work_plane(
                instance.inventor_extent.ToFace, None
            ).to_dict()
    ...
    def from_dict(cls, d: dict):
        instance = cls()
        instance.from_face = d.get("fromFace", {})
        instance.to_face = d.get("toFace", {})
        instance.is_from_face_work_plane = d.get("isFromFaceWorkPlane", False)
        instance.is_to_face_work_plane = d.get("isToFaceWorkPlane", False)
        instance.extend_from_face = d.get("extendFromFace", False)
        instance.extend_to_face = d.get("extendToFace", False)
        return instance
    ...
    def extract_kvs(self) -> dict:
        return {
            "nums": {},
            "enums": {
                KEY["IS_EXTEND_FROM_FACE"]: 1 if self.extend_from_face else 0,
                KEY["IS_EXTEND_TO_FACE"]: 1 if self.extend_to_face else 0,
            },
            "entities": {

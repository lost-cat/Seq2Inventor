...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_root", type=str, default="output", help="输出 JSON 文件的目录路径，默认在零件目录同级创建 output 目录。")
    ...
    data_root = os.path.join(os.path.dirname(part_dir), "output") if args.data_root == "output" else os.path.abspath(args.data_root)
    ...
    with com_sta():
        ...
        try:
            for part_file in pb:
                ...
                ipt_file_dir = os.path.join(data_root, part_name_no_ext)
            ...
            gc.collect()
    # 保存失败文件列表
    failed_reason_json = os.path.join(data_root, "ipt2json_failed_files.json")
